#!/bin/R
# Protein Plotting Script with "Miss" Exclusion and Conservation Trace
...
plot((1:length(scorePlot)), type="n", xlab="Amino Acid Position", ylab="Conservation Score",
     xlim=c(0, length(scorePlot)), ylim=c(0.6, -8), cex.lab=0.9, cex.main=1, yaxt="n", xaxt="n", bty="n",
...
axis(side=1, at=ticks, las=3, line=-3.3)

# Define architecture levels and black bar positions
architecture_levels <- c(-2, -3, -4)
...
plot_architecture_with_black_bar(pa3, miss3, architecture_levels[2], "lightblue", "darkblue", black_bar_levels[2])
plot_architecture_with_black_bar(pa4, miss4, architecture_levels[3], "purple", "darkblue", black_bar_levels[3])
